<script lang="ts">
    ...
    let {
        ...
        width,
        height,
    }: { data: Data; width?: number; height?: number } = $props();
</script>

<div
    class="agent-cluster"
    style="--agent-color: {data.color}; width: {width ??
        0}px; height: {height ?? 0}px;"
>
    ...
    <div class="cluster-header">
        ...
        {#if data.tokens > 0}
            <span class="cluster-tokens"
                >{data.tokens.toLocaleString()} tok</span
            >
        {/if}
    </div>
