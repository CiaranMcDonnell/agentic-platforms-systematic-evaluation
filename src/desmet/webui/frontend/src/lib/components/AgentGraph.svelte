<script lang="ts">
    ...
    let nodes = $derived(
        baseNodes.map((n) => {
            if (n.type === "observation") {
                return {
                    ...n,
                    data: { ...n.data, selected: n.id === selectedObsId },
                };
            }
            return n;
        }),
    ...
    function buildElkNode(
        ...
        const childNodes = sorted.map((c) =>
            buildElkNode(c, depth + 1, nextEdgeId),
        );
    ...
    function emitFlowNodes(
        ...
        if (isAgentContainer) {
            ...
            const agentTotalTokens = agentObsRoot
                ? flattenObservations(agentObsRoot).reduce(
                      (s, o) => s + o.tokens.total,
                      0,
                  )
                : 0;
            out.push({
        ...
        if (hasChildren && !obs) {
            ...
            console.warn(
                "[AgentGraph] compound ELK node not in allObservations, dropping subtree:",
                elkNode.id,
            );
            return;
        }
    ...
    async function loadLangfuseGraph(traceId: string) {
        ...
        try {
            ...
            if (rootObs.length === 1 && rootObs[0].children.length > 0) {
                ...
                const hasCompoundChildren = wrapperChildren.some(
            ...
            const elk = new ELK();
            let edgeCounter = 0;
            const nextEdgeId: EdgeIdGen = (prefix) =>
                `${prefix}-${edgeCounter++}`;
    ...
    let displayTimeline = $derived(
        filterAgentId
            ? (graphData?.timeline ?? []).filter(
                  (e) => e.agent_id === filterAgentId,
              )
            : (graphData?.timeline ?? []),
    );
    ...
    let filterAgentName = $derived(
        filterAgentId
            ? (graphData?.nodes.find((n) => n.id === filterAgentId)?.role ??
                  filterAgentId)
            : null,
    );
    ...
    $effect(() => {
        ...
        edges = graphData.edges.map((e) => {
            ...
            return {
                ...
                data: {
                    dots: visible ? dots : 0,
                    sourceColor: roleColor(e.source),
                },
                style: visible
    ...
    async function layoutFallbackWithELK(
        apiNodes: ApiGraphNode[],
        apiEdges: typeof graphData.edges,
    ) {
        const elk = new ELK();
        const elkGraph = {
            id: "root",
        ...
        return (layout.children ?? []).map((child) => ({
            ...
            data: {
                label:
                    apiNodes.find((n) => n.id === child.id)?.role ?? child.id,
    ...
    async function loadFallbackGraph() {
        ...
        try {
            ...
            baseNodes = await layoutFallbackWithELK(
                graphData.nodes,
                graphData.edges,
            );
            edges = graphData.edges.map((e) => ({
            ...
            error =
                e.status === 404
                    ? "No trace data found."
                    : `Failed to load graph: ${e.message}`;
        } finally {
            loading = false;
        }
    }
        ...
        if (listContainer) {
            observer = new IntersectionObserver(
                (entries) => {
                    ...
                    if (topEntry) {
                        const idx = Number(
                            (topEntry.target as HTMLElement).dataset.eventIndex,
                        );
        ...
        if (mode === "fallback") {
            ...
            if (e.key === "ArrowDown" || e.key === "ArrowUp") {
                ...
                const currentIdx = filtered.findIndex(
                    (ev) => ev.index === selectedEventIndex,
                );
                let nextIdx: number;
                if (e.key === "ArrowDown") {
                    nextIdx =
...
>
    ...
    {:else if mode === "fallback" && graphData && !error}
        <div class="topology-badge">
            ...
            <span class="topology-stats">
                ...
                {graphData.nodes
                    .reduce((s, n) => s + n.tokens_in + n.tokens_out, 0)
                    .toLocaleString()} tokens &middot;
    ...
    <div
        class="graph-layout"
        class:has-drawer={mode === "langfuse" && !!selectedObsId}
    >
            ...
            >
                {#if filterAgentId}
                    <div class="filter-bar">
                        <span
                            >Showing: <strong
                                style="color: {roleColor(filterAgentId)}"
                                >{filterAgentName}</strong
                            >
                            ({displayTimeline.length} messages)</span
                        >
                        <button
                            class="filter-clear"
                            onclick={() => (filterAgentId = null)}
                            >&times;</button
                        >
                    </div>
                {/if}
