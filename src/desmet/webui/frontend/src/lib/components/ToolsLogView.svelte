            ...
            {#each tools as row (row.id)}
                ...
                <div class="tl-row-wrap" class:err={isErr}>
                    <button class="tl-row" onclick={() => toggle(row.id)}>
                        ...
                        >
                        <span class="col-input cell-pre"
                            >{row.input
                                ? row.input.slice(0, 80) +
                                  (row.input.length > 80 ? "…" : "")
                                : "—"}</span
                        >
                        <span class="col-output cell-pre"
                            >{row.output
                        ...
                        >
                        <span class="col-dur"
                            >{(row.duration_ms / 1000).toFixed(3)}s</span
                        >
                        <span class="col-status" class:err-text={isErr}
                            >{isErr ? "ERROR" : "OK"}</span
                        >
                    </button>
                    {#if isOpen}
                        <div class="tl-expand">
                            {#if row.input}
                                <div class="exp-block">
                                    ...
                                    <pre class="exp-pre">{formatContent(
                                            row.input,
                                        )}</pre>
                                </div>
                            {/if}
