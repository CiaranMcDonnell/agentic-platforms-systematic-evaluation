<script lang="ts">
    ...
    $effect(() => {
        ...
        else expanded = depth < 2;
    });

    let barWidth = $derived(
        rootLatency > 0
            ? Math.max(2, (observation.latency_ms / rootLatency) * 100)
            : 0,
    );
...
<div class="span-node" style="margin-left: {depth * 20}px;">
    ...
    >
        ...
        <span class="span-stat"
            >{(observation.latency_ms / 1000).toFixed(2)}s</span
        >
        {#if hasTokens}
            <span class="span-stat tok"
                >{observation.tokens.total.toLocaleString()} tok</span
            >
        {/if}
        ...
        <div class="span-body">
            <div class="span-meta">
                ...
                <div class="meta-cell">
                    ...
                    <span class="meta-value mono"
                        >{(observation.latency_ms / 1000).toFixed(3)}s</span
                    >
                </div>
                {#if hasTokens}
                    <div class="meta-cell">
                        ...
                        <span class="meta-value mono"
                            >{observation.tokens.input.toLocaleString()}</span
                        >
                    </div>
                    <div class="meta-cell">
                        <span class="meta-label">Tokens Out</span>
                        <span class="meta-value mono"
                            >{observation.tokens.output.toLocaleString()}</span
                        >
                    </div>
                {/if}
                {#if observation.model}
                ...
                {#if observation.status_message}
                    <div class="meta-cell wide">
                        ...
                        <span class="meta-value" class:err-text={isError}
                            >{observation.status_message}</span
                        >
                    </div>
                {/if}
            </div>
            ...
            {#if observation.output}
                <details class="io-block">
                    ...
                    <pre class="io-pre">{formatContent(
                            observation.output,
                        )}</pre>
                </details>
            {/if}
