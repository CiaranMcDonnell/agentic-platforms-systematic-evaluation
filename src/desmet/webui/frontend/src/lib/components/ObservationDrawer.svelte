    ...
    >
        <div class="drawer-header">
            <div class="drawer-title">
                <span class="type-badge" style="--type-color: {color}"
                    >{typeLabel(obs.type)}</span
                >
        ...
        <div class="drawer-stats">
            ...
            {#if obs.tokens.total > 0}
                <div class="stat-row">
                    ...
                    <span class="stat-value tokens"
                        >{obs.tokens.input.toLocaleString()}</span
                    >
                </div>
                <div class="stat-row">
                    <span class="stat-label">Tokens Out</span>
                    <span class="stat-value tokens"
                        >{obs.tokens.output.toLocaleString()}</span
                    >
                </div>
            {/if}
            {#if obs.cost > 0}
        ...
        {#if formattedInput}
            <div class="drawer-section">
                ...
                >
                    <span class="chevron" class:open={inputExpanded}
                        >&#9654;</span
                    >
                    Input
                </button>
                {#if inputExpanded}
                    <pre class="section-content">{truncate(
                            formattedInput,
                            inputShowFull,
                        )}</pre>
        ...
        {#if formattedOutput}
            <div class="drawer-section">
                ...
                >
                    <span class="chevron" class:open={outputExpanded}
                        >&#9654;</span
                    >
                    Output
                </button>
