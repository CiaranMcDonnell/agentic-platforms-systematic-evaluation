<script lang="ts">
    ...
    let tokens = $derived(
        traceData
            ? sumTokens(traceData.observations)
            : { input: 0, output: 0, total: 0 },
    );
    let gens = $derived(
        traceData ? countGenerations(traceData.observations) : 0,
    );
    ...
    let totalCost = $derived(
        traceData ? traceData.trace.cost || sumCost(traceData.observations) : 0,
    );

    // Filtered observations for Spans tab
    let filteredObs = $derived(
        traceData ? filterObs(traceData.observations, spanFilter) : [],
    );
    ...
    $effect(() => {
        if (activeTab !== "spans") expandAll = false;
    });
    ...
    function sumTokens(obs: LangfuseObservation[]): {
        input: number;
        output: number;
        total: number;
    } {
...
{:else if traceData}
    <div class="trace-wrap">
        ...
        <div class="trace-pills">
            <div class="pill">
                <span class="pill-lbl">Tokens In</span><span class="pill-val"
                    >{tokens.input.toLocaleString()}</span
                >
            </div>
            <div class="pill">
                <span class="pill-lbl">Tokens Out</span><span class="pill-val"
                    >{tokens.output.toLocaleString()}</span
                >
            </div>
            <div class="pill">
                <span class="pill-lbl">Total Tokens</span><span class="pill-val"
                    >{tokens.total.toLocaleString()}</span
                >
            </div>
            {#if totalCost > 0}
                <div class="pill">
                    <span class="pill-lbl">Est. Cost</span><span
                        class="pill-val">${totalCost.toFixed(4)}</span
                    >
                </div>
            {/if}
            <div class="pill">
                <span class="pill-lbl">LLM Calls</span><span class="pill-val"
                    >{gens}</span
                >
            </div>
            <div class="pill">
                <span class="pill-lbl">Spans</span><span class="pill-val"
                    >{spanCount}</span
                >
            </div>
            {#if errs > 0}
                <div class="pill pill-err">
                    <span class="pill-lbl">Errors</span><span class="pill-val"
                        >{errs}</span
                    >
                </div>
            {/if}
