...
Measures the framework's orchestration overhead relative to the story's time budget. This captures how much time the framework spends on coordination, parsing, and retries beyond what the LLM calls themselves require.
...
*Evaluation guidance:* This dimension is computed automatically from `wall_clock_seconds / time_budget_seconds` recorded in the story metrics. Manual scoring is only required when the automatic ratio does not capture relevant context (e.g.\ network latency inflating the time for a specific run).
...
The six rubric dimensions are aggregated into four cross-cutting scores on a 1--5 Likert scale. The mapping is:
...
#figure(
  table(
    ...
    [Pipeline Completeness], [Story completion ratio (automatic) + pipeline completeness rubric score],
...
The formulas for this aggregation are defined in the Project Approach and Design chapter. Automatic metrics (completion ratio, token counts, timing, intervention counts) are captured by the harness during execution. Rubric scores are assigned manually by the evaluator via the management console's scoring panel after reviewing execution traces and Langfuse observations.
