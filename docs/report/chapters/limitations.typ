...
*Adapter-level measurement fidelity.* A structured review of all five implemented SDK and multi-agent adapters identified and addressed several classes of bug that could bias cross-platform comparisons: silent event-drop in the CrewAI adapter when the same adapter instance serviced more than one story (class-level handler registration), cumulative iteration counters that inflated `trace.total_iterations` with run order, guardrail-only validation in the OpenAI Agents SDK adapter that let executor-final-output runs report success without a workspace check, missing asymmetric-tool distribution in the same adapter (reviewer received write tools), silently-swallowed planner fallbacks in all five adapters (`except Exception: pass`) that obscured structured-output failures, an over-defensive regex escape in the Google ADK adapter that relied on ADK's own regex to backstop nested braces, and token/duration double-counting in the LangGraph (reviewer duration catch-up, structured-planner usage discarded) and Microsoft Agent Framework (wall-clock catch-up on top of middleware-recorded per-call durations) adapters. All fixes are accompanied by regression tests and landed in branch `fix/adapter-scoring-accuracy`. Two further adapter concerns flagged by the review (`raw_responses` inflation across retries in the OpenAI SDK adapter; streamed `usage_metadata` summation in the ADK adapter) were investigated against the installed SDK versions and found not to manifest under the harness's current configuration: the OpenAI Agents SDK 0.4.2 `Runner.run` initialises a fresh per-call `model_responses` list, and the ADK adapter uses the default non-streaming `RunConfig` so Gemini yields a single final `usage_metadata` per LLM call rather than cumulative partials. Three cross-adapter concerns are intentionally deferred as methodology decisions rather than bug fixes: iteration-count semantics differ across adapters (LangGraph counts graph-node steps; CrewAI counts LLM calls including tool sub-calls; MAF counts executor turns with manager turns excluded; OpenAI SDK counts `new_items`; ADK counts events with author, inflating 3--5× relative to peers) and have not been normalised, CrewAI's outer-retry loop multiplies effective iteration budget by up to 3× versus peers, and LangGraph's reviewer subgraph is given tools via `bind_tools` but has no tool-execution node so those calls are silently dropped. These three are documented here as known cross-platform comparability gaps that affect Efficiency, Autonomy, and Orchestration rubric dimensions; per-platform absolute numbers remain interpretable, but relative rankings on these dimensions should be read with the qualification that measurement units are not uniform across adapter families.
