...
DESMET provides criteria to help evaluators select an appropriate method based on specific circumstances, including whether benefits are clearly quantifiable, availability of staff for experiments, stability of development procedures, timescales available, user population size, and whether benefits are observable from task output. @fig-desmet-selection illustrates how these criteria applied to this study led to a hybrid approach combining Qualitative Screening (Layers~1--2) with Hybrid Benchmarking (Layer~3): the expected benefits of agentic tools resist precise quantification, eliminating purely quantitative methods; the absence of a large accessible user population rules out survey-based approaches; the architectural heterogeneity of the nine platforms favours qualitative feature screening; and the availability of standardised software engineering tasks enables objective benchmarking.
...
The analysis reveals that while many platforms offer no-code interfaces and remote LLM integration, few combine interoperability, extensibility, and explicit human-in-the-loop modelling. Most tools remain tightly coupled to specific SDKs such as LangChain, limiting long-term adaptability. Execution monitoring remains limited, with only basic logging rather than full observability. Human-in-the-loop functionality is restricted in most tools---while some allow pausing for manual input, these capabilities are not modelled as first-class workflow elements. Broccia et al.'s analysis is limited to visual workflow platforms and excludes multi-agent frameworks (LangGraph, CrewAI) and SDK runtimes (OpenAI Agents SDK, Google ADK, Microsoft Agent Framework). Their feature analysis also omits pipeline-based benchmarking---platforms are compared on what they _offer_, not how they _perform_ on real tasks. The present study extends their framework to all three categories and adds empirical benchmarking as a third evaluation layer.
