...
The evaluation covers nine agentic platforms representing multi-agent frameworks (LangGraph, CrewAI), agent SDK runtimes (OpenAI Agents SDK, Google ADK, Microsoft Agent Framework), and visual workflow platforms (Flowise, LangFlow, Dify, N8n). All nine platforms are assessed at Layers 1 and 2 through desk research and hands-on feature verification. Layer 3 pipeline benchmarking is conducted for eight platforms---LangGraph, CrewAI, OpenAI Agents SDK, Google ADK, Microsoft Agent Framework, Flowise, LangFlow, and N8n---using purpose-built adapters. Dify is covered at Layers 1--2 only as a partial integration: its init, authentication, and app-creation automate cleanly, but end-to-end execution on Dify ≥1.13 is blocked by a marketplace-only model-provider ecosystem that requires out-of-band plugin installation (discussed as a finding in @limitations). Exhaustive platform coverage, longitudinal studies, economic cost-benefit analysis, and proprietary tools (e.g., Devin) are out of scope.
